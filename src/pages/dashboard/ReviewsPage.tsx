...
export const ReviewsPage: React.FC = () => {
  ...
  const handleApproveReview = async (reviewId: string) => {
    try {
      ...
      console.log('Review approved, refreshing data...');
      // Refresh the entire reviews list to get the latest data
      await refreshReviews();

      console.log('Review approved and data refreshed successfully');
    } catch (error) {
  ...
  const handleRejectReview = async (reviewId: string) => {
    try {
      ...
      console.log('Review rejected, refreshing data...');
      // Refresh the entire reviews list to get the latest data
      await refreshReviews();

      console.log('Review rejected and data refreshed successfully');
    } catch (error) {
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-900 border-b border-gray-800">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h1 className="text-4xl font-bold text-white mb-2">
                {user?.role === 'business' ? 'Campaign' : 'Your'} <span className="text-primary">Reviews</span>
              </h1>
              <p className="text-xl text-gray-300">
                {user?.role === 'business' ? 'Monitor and manage reviews for your campaigns' : 'Track your review submissions and earnings'}
              </p>
            </div>
          </div>
        </div>
